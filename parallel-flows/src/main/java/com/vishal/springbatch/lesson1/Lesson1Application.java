...
public class Lesson1Application {
	...
	// creating a listener bean to add on a step
	@Bean
	public StepExecutionListener selectFlowerListener() {
		return new FlowersSelectionStepExecutionListener();
	}

	// externalizing delivery flow to package job and delivery job
	@Bean
	public Flow deliveryFlow() {
		return new FlowBuilder<SimpleFlow>("deliveryFlow").next(driveToAddressStep())
				.on("FAILED").fail()
	...
	// For nested jobing we need to create a job step
	@Bean
