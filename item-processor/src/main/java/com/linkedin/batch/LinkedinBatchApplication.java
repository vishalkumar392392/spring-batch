...
//		return new JsonFileItemWriterBuilder<Order>().jsonObjectMarshaller(new JacksonJsonObjectMarshaller<Order>())
//				.resource(new FileSystemResource("shipped_orders_output.json")).name("jsonItemWriter").build();
//	}
...
//				.processor(orderValidatingItemProcessor()).writer(itemWriter()).build();
//	}

//	@Bean
//	public Step chunkBasedStep() throws Exception {
//		return this.stepBuilderFactory.get("chunkBasedStep").<Order, TrackedOrder>chunk(10).reader(itemReader())
//				.processor(trackedOrderItemProcessor()).writer(itemWriter()).build();
//	}

	@Bean
	public Step chunkBasedStep() throws Exception {
		return this.stepBuilderFactory.get("chunkBasedStep").<Order, TrackedOrder>chunk(10).reader(itemReader())
				.processor(compositeItemProcessor()).writer(itemWriter()).build();
	}
	...
	public ItemProcessor<Order, TrackedOrder> trackedOrderItemProcessor() {
		return new TrackedItemProcessor();
	}
	...
	public Job job() throws Exception {
		return this.jobBuilderFactory.get("job2").start(chunkBasedStep()).build();
	}
